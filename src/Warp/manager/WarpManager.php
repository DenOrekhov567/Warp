...
use Warp\exception\WarpException;
...
class WarpManager
{
    ...
    public static function initData(): void {
        ...
        if($result = SQL::getConnection()->query($sql)){
    ...
    public static function setWarp(array $data): void {
        ...
        $jsonData->x = (int)$data["x"];
        $jsonData->y = (int)$data["y"];
        $jsonData->z = (int)$data["z"];
    ...
    public static function saveData(): void {
        foreach(self::$warps as $warp) {
            if(SQL::getConnection()->query("SELECT *  FROM `warps` WHERE `name` = '".$warp->getName()."'")->fetch_assoc() === null) {
                SQL::getConnection()->query("INSERT INTO `warps`(`name`, `json`) VALUES (
                    '".$warp->getName()."',  
                    '".json_encode($warp->data)."'
                )");
